...
var Walker = function () {
  ...
  _createClass(Walker, [{
    ...
    value: function _walk_down(name, range, version, path, dependency_node) {
      ...
      var package_range_id = parse_module_id(name, range).pkg;
      var package_id = parse_module_id(name, version).pkg;
      ...
      dependencies.forEach(function (dep) {
        ...
        var dep_name = _parse_module_id.dep_name;
        var dep_range = _parse_module_id.dep_range;
        var dep_path = _parse_module_id.dep_path;

        // The dependency version of a package is already resolved by
        //   neuron-package-dependency

        var dep_version = dependencies[dep];
        _this2._walk_down_non_facade(dep_name, dep_range, dep_version, current_dependency_node);
      });
    }
  }, {
